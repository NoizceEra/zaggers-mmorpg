...
## Files changed
- `assets/sprites_ff/hero_monk.png`, `assets/sprites_ff/hero_thief.png` (thief fix; monk fix
  superseded by the master merge below, see note)
...
## Post-merge note: master moved to 11 classes mid-task

While this pass was in progress, `master` gained two new commits (`377f7fa` "6 new hero
classes (11 total), 2.5D sprite sheets, and Stat-Driven Class Advancement system" and
`50dde27` "66 skills and 11 classes schema update") that were not yet in `integration`.
Per instructions, merged `master` into this branch (commit `f1d7451`) favoring additive
resolution:

- **`web/index.html` conflict**: both branches inserted a new function in the same spot
  (right after `buyItem()`). HEAD added `flashBarOnDrop()` (HP/SP bar-pulse helper);
  master added `updateJobAdvancementUI()` + `promoteJobClass()` (job-advancement UI for
  the new 11-class system). Resolved additively - kept all three functions, each with
  its own closing brace (the raw conflict had merged their braces together, which would
  have been a syntax error left unresolved).
- **`assets/sprites_ff/hero_monk.png` / `web/assets/sprites_ff/hero_monk.png` conflict**:
  master's `377f7fa` replaced `hero_monk.png` with a newly-generated, much simpler
  placeholder sprite (`scripts/generate_new_hero_sprites.py`, ~2.2KB vs. the original
  ~10.9KB hand-drawn sheet this pass had just fixed). Took master's version (the newer,
  actively-developed art direction) rather than my patched copy of the old asset, since
  keeping the old detailed art would have silently reverted master's newer class-art
  pipeline. **Re-audited master's new placeholder for the same row-duplication bug** and
  it does *not* have it: Left/Right rows are a proper mirrored pair (verified visually
  and by diff) and Up is a distinct back-of-head view. My thief fix was unaffected
  (master never touched `hero_thief.png`).
- **New classes' sprites audited too**: since master's merge brought the DB to 11 classes
  (`dragoon`, `monk`, `paladin`, `sage`, `bard`, `alchemist` sprites all newly generated
  or regenerated by the same script), ran the same row-duplicate diff audit across all
  six. All six are structurally consistent with each other (identical diff signature)
  and visually confirmed correct (Down symmetric front view, Left/Right a mirrored pair
  with a distinguishing hair/detail swap, Up a faceless back view) - none exhibit the
  bug #1 pattern. No further sprite regeneration was needed.
- Re-ran the `node --check` syntax validation and the sprite-dimension audit after the
  merge; both pass, and all this pass's monster-animation and DB-caching changes in
  `web/index.html` survived the merge intact (verified by re-grepping for
  `attackPoseTimer` and the `ensureGameDB` fetch-first comment post-merge).
- Verified all 11 classes in the merged `zaggers_database.json` resolve to sprite files
  that exist in both `assets/sprites_ff/` and `web/assets/sprites_ff/`.
