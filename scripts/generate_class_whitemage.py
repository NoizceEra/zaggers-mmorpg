...
def draw_whitemage_frame(draw, row, col):
    ...
    draw.polygon([(18, 28 + cy), (46, 28 + cy), (48, 54 + cy), (16, 54 + cy)], fill=WHITE_4, outline=OUTLINE)
    draw.polygon([(18, 28 + cy), (32, 54 + cy), (16, 54 + cy)], fill=WHITE_5)
    draw.polygon([(16, 48 + cy), (22, 54 + cy), (16, 54 + cy)], fill=WHITE_6)
    draw.polygon([(36, 30 + cy), (46, 28 + cy), (44, 48 + cy)], fill=WHITE_3)
    # Front Robe Slit
    draw.polygon([(29, 40 + cy), (35, 40 + cy), (37, 55 + cy), (27, 55 + cy)], fill=(20, 15, 25, 255))
    draw.line([(25, 28 + cy), (27, 52 + cy)], fill=WHITE_1, width=1)
    draw.line([(17, 29 + cy), (17, 54 + cy)], fill=WHITE_1, width=1) # Rim light
    ...
    # TIGHT ARTICULATED BELL SLEEVES & WHITE GLOVED HANDS
    draw.polygon([(15, 26 + cy), (22, 27 + cy), (20, 37 + cy), (14, 34 + cy)], fill=WHITE_3, outline=OUTLINE)
    draw.rectangle([14, 35 + cy, 21, 44 + cy], fill=WHITE_5, outline=OUTLINE) # Bell sleeve
    draw.rectangle([14, 42 + cy, 20, 47 + cy], fill=WHITE_1, outline=OUTLINE) # White gloved hand

    draw.polygon([(49, 26 + cy), (42, 27 + cy), (44, 37 + cy), (50, 34 + cy)], fill=WHITE_4, outline=OUTLINE)
    draw.rectangle([43, 35 + cy, 50, 44 + cy], fill=WHITE_6, outline=OUTLINE) # Bell sleeve
    draw.rectangle([44, 42 + cy, 50, 47 + cy], fill=WHITE_1, outline=OUTLINE) # White gloved hand
